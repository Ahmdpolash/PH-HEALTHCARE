...
"use client";
import { getTimeIn12HourFormat } from "@/app/(dashboardLayout)/dashboard/_component/MultipleSelectField";
...
const DoctorScheduleSlots = ({ id }: { id: string }) => {
  ...
  //   const { data, } = useGetAllDoctorScheduleQuery({ ...query });
  const { data: doctorSchedules, isLoading } = useGetMyScheduleQuery({});
  ...
  const { data: schedulesOfTomorrow, isLoading: loading } =
    useGetMyScheduleQuery({});
